...
---
# <a name="resource-dependencies-using-dependson"></a><span data-ttu-id="2d677-103">DependsOn kullanan kaynak bağımlılıkları</span><span class="sxs-lookup"><span data-stu-id="2d677-103">Resource dependencies using DependsOn</span></span>

<span data-ttu-id="2d677-104">Yazdığınızda [yapılandırmaları](configurations.md), eklediğiniz [kaynak bloklar](../resources/resources.md) hedef düğümü yönlerini yapılandırmak için.</span><span class="sxs-lookup"><span data-stu-id="2d677-104">When you write [Configurations](configurations.md), you add [Resource blocks](../resources/resources.md) to configure aspects of a target Node.</span></span> <span data-ttu-id="2d677-105">Kaynak bloklar eklemeye devam ederken, yapılandırmalarınızı oldukça büyük ve yönetmek için hantal büyüyebilir.</span><span class="sxs-lookup"><span data-stu-id="2d677-105">As you continue to add Resource blocks, your Configurations can grow quite large and cumbersome to manage.</span></span> <span data-ttu-id="2d677-106">Bir challenge, kaynak bloğu uygulanan sırasıdır.</span><span class="sxs-lookup"><span data-stu-id="2d677-106">One such challenge is the applied order of your resource blocks.</span></span> <span data-ttu-id="2d677-107">Genellikle kaynakları yapılandırma içinde tanımlandıkları sırayla uygulanır.</span><span class="sxs-lookup"><span data-stu-id="2d677-107">Typically resources are applied in the order they are defined within the Configuration.</span></span> <span data-ttu-id="2d677-108">Yapılandırmanızı daha büyük ve daha karmaşık büyüdükçe, kullanabileceğiniz `DependsOn` kaynaklarınızın bir kaynak üzerinde başka bir kaynağa bağlı belirterek uygulanan sırasını değiştirmek için anahtar.</span><span class="sxs-lookup"><span data-stu-id="2d677-108">As your Configuration grows larger and more complex, you can use the `DependsOn` key to change the applied order of your resources by specifying that a resource depends on another resource.</span></span>

<span data-ttu-id="2d677-109">`DependsOn` Anahtarı herhangi bir kaynak bloğu içinde kullanılabilir.</span><span class="sxs-lookup"><span data-stu-id="2d677-109">The `DependsOn` key can be used in any Resource block.</span></span> <span data-ttu-id="2d677-110">Diğer kaynak anahtarı olarak aynı anahtar/değer mekanizması ile tanımlanır.</span><span class="sxs-lookup"><span data-stu-id="2d677-110">It is defined with the same key/value mechanism as other Resource keys.</span></span> <span data-ttu-id="2d677-111">`DependsOn` Anahtarı, aşağıdaki sözdizimini kullanarak bir dize dizisi bekliyor.</span><span class="sxs-lookup"><span data-stu-id="2d677-111">The `DependsOn` key expects an array of strings with the following syntax.</span></span>

```
DependsOn = '[<Resource Type>]<Resource Name>', '[<Resource Type>]<Resource Name'
```

<span data-ttu-id="2d677-112">Aşağıdaki örnek, etkinleştirme ve genel profil yapılandırdıktan sonra bir güvenlik duvarı kuralı yapılandırır.</span><span class="sxs-lookup"><span data-stu-id="2d677-112">The following example configures a firewall rule after enabling and configuring the public profile.</span></span>
...
<span data-ttu-id="2d677-113">Yapılandırmayı uygulamak, güvenlik duvarı profili sırasını bağımsız olarak kaynak bloklar önce tanımlanan her zaman yapılandırılır.</span><span class="sxs-lookup"><span data-stu-id="2d677-113">When you apply the Configuration, the firewall profile will always be configured first regardless of which order the Resource blocks are defined.</span></span> <span data-ttu-id="2d677-114">Yapılandırma uygularsanız, isterseniz geri alabilmenizi yapılandırma mevcut hedef düğümlerinizi not etmeyi unutmayın.</span><span class="sxs-lookup"><span data-stu-id="2d677-114">If you apply the Configuration, be sure to note your target Nodes existing Configuration so you can revert if desired.</span></span>
...
<span data-ttu-id="2d677-115">Bu durumlarda da sağlar **FirewallProfile** kaynak herhangi bir nedenle başarısız **Güvenlik Duvarı** blok değil yürütülecek rağmen ilk kez tanımlandı.</span><span class="sxs-lookup"><span data-stu-id="2d677-115">This also ensures that if the **FirewallProfile** resource fails for any reason, the **Firewall** block will not execute even though it was defined first.</span></span> <span data-ttu-id="2d677-116">`DependsOn` Anahtar gruplandırma blokları kaynak ve kaynak yürütülmeden önce bağımlılıkları çözümlenen emin olunması daha fazla esneklik sağlar.</span><span class="sxs-lookup"><span data-stu-id="2d677-116">The `DependsOn` key allows more flexibility in grouping resource blocks and ensuring that dependencies are resolved before a Resource executes.</span></span>

<span data-ttu-id="2d677-117">Daha gelişmiş yapılandırmaları içinde de kullanabilirsiniz [çapraz düğüm bağımlılığı](crossNodeDependencies.md) daha ayrıntılı bir denetim (örneğin, bir etki alanı denetleyicisi yapılandırılmış bir istemci etki alanına katılmadan önce sağlama) izin vermek için.</span><span class="sxs-lookup"><span data-stu-id="2d677-117">In more advanced Configurations, you can also use [Cross Node Dependency](crossNodeDependencies.md) to allow even more granular control (For example, ensuring a domain controller is configured before joining a client to the domain).</span></span>

## <a name="cleaning-up"></a><span data-ttu-id="2d677-118">Temizleme</span><span class="sxs-lookup"><span data-stu-id="2d677-118">Cleaning Up</span></span>

<span data-ttu-id="2d677-119">Yukarıdaki yapılandırma uyguladıysanız, tüm değişiklikleri geri almak için anahtarları tersine çevirebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="2d677-119">If you applied the Configuration above, you can reverse keys to undo any changes.</span></span> <span data-ttu-id="2d677-120">Yukarıdaki örnekte ayarlama **etkin** false anahtarı devre dışı bırakır güvenlik duvarı kuralı ve profili.</span><span class="sxs-lookup"><span data-stu-id="2d677-120">In the above example, setting the **Enabled** key to false will disable the firewall rule and profile.</span></span> <span data-ttu-id="2d677-121">Örnek hedef düğümün önceki yapılandırılmış durum uyacak şekilde değiştirmeniz gerekir.</span><span class="sxs-lookup"><span data-stu-id="2d677-121">You should modify the example as needed to match your target Node's previous configured state.</span></span>
...
- [<span data-ttu-id="2d677-123">Çapraz düğüm bağımlılıklarını kullanın</span><span class="sxs-lookup"><span data-stu-id="2d677-123">Use Cross Node Dependencies</span></span>](./crossNodeDependencies.md)
